...
def main():
    ...
    c = curse.Curse()
    c.update_status_line("w: view working copy status, r: browse remote repo, "
                         "q: quit")
...
def browse_repo(c, client, working_copy):
    c.update_status_line("loading...")
    d = svn_dir.Dir(client)
    files = d.ls(working_copy)
    if files is None:
        c.update_status_line(working_copy + " - Not under version control. "
                             "q: quit")
    else:
        c.update_status_line(working_copy + " - j/k: up/down, q: quit")
...
def login_handler(username=USERNAME):
    ...
    https://tools.ietf.org/doc/python-svn/pysvn_prog_ref.html#pysvn_client_callback_get_login
    """
    if not username:
